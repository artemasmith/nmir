class ParserAdresat
  ...
  def self.parse_category row
     case
       when row[0].mb_chars.downcase.to_s.match(/квартир/) then :flat
       when row[0].mb_chars.downcase.to_s.match(/дом/) then :house
       when row[0].mb_chars.downcase.to_s.match(/участ[ки|ок]{0,1}/) then :ijs
       else nil
     end
  end
  ...
  def self.parse_offer_type row
    case
      when row[0].mb_chars.downcase.to_s.match(/прода/) then :sale
      else nil
    end
  end
  ...
  def self.parse_adv_type row
    case
      when row[0].mb_chars.downcase.to_s.match(/прода/) then :offer
      when row[0].mb_chars.downcase.to_s.match(/покуп/) then :buy
      else nil
    end
  end
  ...
  def self.parse_name_and_phone row, titles
    #begin
      ...
      name = unparsed_contact[0].gsub(/[-+\(\)\,\s]/, '') if unparsed_contact[0].match(/[A-Za-z_А-Яа-я]/)
      return name, phone
    # rescue
    #   return nil, nil
    # end
  ...
  def self.parse_comment row, titles
    ...
    comment << "отд.хар: " + self.parse_char(row, titles) if titles['отд.хар'].present? && row[titles['отд.хар']].present?
    comment << "год: " + self.parse_year(row, titles) if titles['год'].present? && row[titles['год']].present?
    comment << "бал: " + self.parse_balcony(row, titles) if titles['бал'].present? && row[titles['бал']].present?
    ...
    comment << "су: " + self.parse_bath(row, titles) if titles['су'].present? && row[titles['су']].present?
    comment << "кан: " + self.parse_plumbing(row, titles) if titles['кан'].present? && row[titles['кан']].present?
    comment << "газ: " + self.parse_gas(row, titles) if titles['газ'].present? && row[titles['газ']].present?
    comment << "зем.под: " + self.parse_land_status(row, titles) if titles['зем.под'].present? && row[titles['зем.под']].present?
    comment << "отд.кач: " + self.parse_renovation(row, titles) if titles['отд.кач'].present? && row[titles['отд.кач']].present?
    comment << "план.1: " + self.parse_blueprint(row, titles) if titles['план.1'].present? && row[titles['план.1']].present?
    ...
    comment << "обособл: " + self.parse_standalone(row, titles) if titles['обособл'].present? && row[titles['обособл']].present?
    uc = row[titles[:comment]].split('|')[2]
    uc = ParserUtil.rename :comment, uc
    comment << "comment: " + uc if uc != '0' && uc.present?
    comment.join('; ')
  end
  ...
  def self.method_missing(name, *args)
    begin
      ...
      #return nil if args[0].blank? || args[1].blank? || args[0][args[1][field]].blank? || args[0][args[1][field]].send(type) == 0

      result = args[0][args[1][field]].strip.send(type)
