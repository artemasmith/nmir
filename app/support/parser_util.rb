class ParserUtil
  ...
  def self.rename field, name
    ...
    hash_list =
    {
      :address =>
        {
          ...
          '1 Ордж' => 'Орджоникидзе',
          '2 Ордж' => 'Орджоникидзе',
          '1пос.Орджон' => 'Орджоникидзе',
          '2пос.Орджон' => 'Орджоникидзе',
          'Зооп.' => 'Зоопарк',
          'Военв.' => 'Военвед',
          'Алекс.' => 'Александровка',
          ...
          '40Лет Победы' => 'пр-кт 40-летия Победы',
          '40Летия Победы' => 'пр-кт 40-летия Победы',
          'Шолохова' => 'пр-кт Шолохова',
          'Добровольского' => 'ул Добровольского',
          'Добровольского' => 'ул Добровольского',
          'п. Койсуг' => 'п Койсуг',
          'район ЦГБ' => 'Центр',
          'Н.поселение' => 'Новое поселение'
      ...
      :blueprint =>
        {
          ...
          'смежн' => 'смежные',
          'вагон' => 'вагон',
          'смежИзол' => 'смежный-Изолированные',
          '1+2' => '1+2',
          'распаш' => 'распашонка'
        },
      :standalone =>
      ...
      :parking =>
        {
          'гр а/м' => 'грузового автомобиля',
          'лег а/м' => 'легкового автомобиля',
          'газ' => 'газели',
        },
      :facilities =>
        {
  ...
  def self.find_child parent, title, type = :all
    result = parent.children_locations(type).where('title ~* ?', "#{title
                                                               .to_s
                                                               .mb_chars
                                                               .downcase
                                                               .gsub('е', '[еи]')
                                                               .gsub('ё', '[её]')
                                                               .gsub('о', '[оа]')}").limit(2)
    pl = result.find{|e| e.title =~ /^пл\s/ }
    pr = result.find{|e| e.title =~ /^пр-кт\s/ }
    return pr if pl.present? && pr.present?
    return result.first
  end
